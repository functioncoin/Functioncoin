...
void MiningPage::startPoolMining()
{
    ...
    args << "-a" << "heavy"; // --algo=ALGO specify the algorithm to use heavycoin hash
    args << "-v" << ui->voteBox->text().toLatin1();
    ...
    args << "-r" << "5"; // --retries=N number of times to retry if a network call fails
    args << "-R" << "20";// --retry-pause=N time to pause between retries, in seconds <default:30>
...
void MiningPage::saveSettings()
{
    ...
    clientModel->setMiningVote(ui->voteBox->value());
...
void MiningPage::loadSettings()
{
    ...
    ui->voteBox->setValue(clientModel->getMiningVote());
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList(tr("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as Functioncoin-Qt."), ERROR, NULL);
    }
}
